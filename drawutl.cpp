	...
	)
{
	...
	!a_bTransparent ? FillSolidRect(a_hDC, l_rect, a_clrs.clrBk) : 0;
...
// Loads  image from resources and converts it to HBITMAP - generated by CoPilot
HBITMAP LoadImageFromResource(HINSTANCE a_hInstance, LPCWSTR a_sResourceName, LPCWSTR a_sResourceType) 
{
	// Step 1: Load resource
	HRSRC l_hRes = ::FindResource(a_hInstance, a_sResourceName, a_sResourceType); // doesn't need to be released
	if (!l_hRes) 
		return nullptr;
...
#ifndef _WIN32_WCE
int DrawTextRotated(HDC a_hDC, LPTSTR a_psText, int a_iLen, const LPPOINT a_pPt, const LPSIZE a_pSize, UINT a_iFmt, int a_iAngle)
{
